...
module archer_rv32i_single_cycle (
    ...
    localparam FLOAT_ADD = 2'b00;
    localparam FLOAT_SUB = 2'b01;
    localparam FLOAT_MUL = 2'b10; // Added since even after telling the compiler to compile archerdefs.v first, those vars are not defined.
    ...
    assign instruction_type = (imem_dataout[6:0] == 7'b1010011 && imem_dataout[14:12] == 3'b000 && imem_dataout[31:25] == 7'b0000000) ? FLOAT_ADD :
                              (imem_dataout[6:0] == 7'b1010011 && imem_dataout[14:12] == 3'b000 && imem_dataout[31:25] == 7'b0000100) ? FLOAT_SUB :
                              (imem_dataout[6:0] == 7'b1010011 && imem_dataout[14:12] == 3'b000 && imem_dataout[31:25] == 7'b0001000) ? FLOAT_MUL :
                              DEFAULT;
    
    assign is_fp_op = (instruction_type == FLOAT_ADD) || 
                      (instruction_type == FLOAT_SUB) ||
                      (instruction_type == FLOAT_MUL);
    
    
    assign fpu_opcode = (instruction_type == FLOAT_ADD) ? 2'b00 :
                        (instruction_type == FLOAT_SUB) ? 2'b01 :
                        (instruction_type == FLOAT_MUL) ? 2'b10 :
                        2'b00; // default
    
    FPU my_fpu (
        .op1(rs1_data),     // first operand
        .op2(rs2_data),     // second operand
        .opcode(fpu_opcode), // new signal you define below
        .result(fpu_result)
    );
